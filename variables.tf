...
EOT
  ...
  validation {
    condition = alltrue([
      for k, v in var.resource_group_policy_exemptions : (
        length(v.name) > 0
      )
    ])
  ...
  # --- Unconfirmed validation candidates, derived from azurerm_resource_group_policy_exemption's provider source ---
